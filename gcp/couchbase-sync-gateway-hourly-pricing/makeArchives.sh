...
function makeArchive()
{
  ...
  cp -r "${dir}resources" "$dir../../build/gcp/couchbase-sync-gateway-hourly-pricing/package"
  # Need to perform the replacement on the group.py for the script_url
  bash "$dir../../script_url_replacer.sh" "$dir../../build/gcp/couchbase-sync-gateway-hourly-pricing/package/group.py"
  zip -r -j -X "$dir../../build/gcp/couchbase-sync-gateway-hourly-pricing/gcp-sg-archive-${license}.zip" "$dir../../build/gcp/couchbase-sync-gateway-hourly-pricing/package"
  #rm -rf "$dir../../build/tmp"
}
