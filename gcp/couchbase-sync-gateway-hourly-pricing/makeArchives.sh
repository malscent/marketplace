...
{
  license=$1
  mkdir -p ../../build/gcp/couchbase-sync-gateway-hourly-pricing/
  rm ../../build/gcp/couchbase-sync-gateway-hourly-pricing/archive-${license}.zip
  mkdir -p ../../build/tmp
  ...
  zip -r -X ../../build/gcp/couchbase-sync-gateway-hourly-pricing/archive-${license}.zip ../../build/tmp
  rm -rf ../../build/tmp
}
